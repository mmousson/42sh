#!/bin/bash
printf "=== TESTING GROUPED COMMANDS =============================================================\n"
bkp_pwd=`pwd`
cd grouped 2>/dev/null
rm -f /tmp/grouped_memory_report
rm -rf /tmp/outputs
...
for cmd in command_files/*; do
	...
	then
		...
		test -s /tmp/log && echo "=== REPORT OF COMMAND: "$str >> /tmp/grouped_memory_report && cat /tmp/log >> /tmp/grouped_memory_report
	elif [ "$#" -eq 1 ]
	then
